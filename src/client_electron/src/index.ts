...
ipcMain.handle('readRobotsConfig', async () => {
  try {
...
ipcMain.handle('writeRobotsConfig', async (_, robotsData) => {
  try {
    const configPath = getConfigPath();
    await writeFile(configPath, JSON.stringify(robotsData, null, 2), 'utf8');
  } catch (error) {
