...
export const useRobotTranslations = () => {
  ...
  const getRobotDisplayName = useCallback((robot: Robot): string => {
    return t('robot.robotNameDefault', 'Robot {{id}}', { id: robot.id });
  }, [t]);

  const translateFeedbackMessage = useCallback((messageKey: string, messageParams?: Record<string, any>): string => {
